var DonatBoss = (() => {
  ...
  var getBrandLogo = () => {
    try {
      return localStorage.getItem("branding_logo_url") || "./logo.jpg";
    } catch {
      return "./logo.jpg";
    }
  };
  var setBrandLogoLocal = (url) => {
    try {
      if (url) localStorage.setItem("branding_logo_url", url);
    } catch {
    }
  };
  ...
  var uploadAsset = async (file, folder = "menu") => {
    ...
    if (!bucket || bucket === "ganti_dengan_nama_bucket_kamu") {
      throw new Error("Isi SUPABASE_ASSET_BUCKET di config.js dulu.");
    }
  ...
  var getMenuHPPBreakdown = (menu) => {
    ...
    const bahanPokokPerPcs = (menu.resepBahanPokok || []).reduce((a, r) => {
      const b = bahan.find((x) => x.id === r.bahanId);
      if (!b) return a;
      const hpg = b.satuan === "kg" || b.satuan === "liter" ? b.harga / 1e3 : b.harga;
      return a + hpg * r.gram;
    }, 0);
    const varianPerPcs = (menu.resepToping || []).reduce((a, t) => a + (t.harga || 0), 0);
    const produkPerPcs = Math.ceil(bahanPokokPerPcs + varianPerPcs);
    const isiBox = Math.max(parseInt(menu.isiBox || 1) || 1, 1);
    const boxCost = Math.ceil(parseFloat(menu.boxCost || 0) || 0);
    const produkPerBox = Math.ceil(produkPerPcs * isiBox);
    const totalBox = Math.ceil(produkPerBox + boxCost);
    return {
      bahanPokokPerPcs: Math.ceil(bahanPokokPerPcs),
      varianPerPcs: Math.ceil(varianPerPcs),
      produkPerPcs,
      isiBox,
      boxCost,
      produkPerBox,
      totalBox,
      marginPerPcs: Math.ceil((parseFloat(menu.hargaJual || 0) || 0) - produkPerPcs),
      marginPerBox: Math.ceil((parseFloat(menu.hargaJual || 0) || 0) - totalBox)
    };
  };
  var hitungHPP = (menu) => {
    const info = getMenuHPPBreakdown(menu);
    return menu?.tipe === "paket" ? info.totalBox : info.produkPerPcs;
  };
  ...
  function WorkerPage({ pushNotif, me, mode = "worker" }) {
    ...
    const selectedAbs = useMemo(() => {
      ...
      return all.find((a) => a.user_id === userId && a.date === txDate) || null;
    }, [tick, userId, txDate]);
    ...
    const doCheckin = () => {
      ...
      const namaHariIni = ["Minggu", "Senin", "Selasa", "Rabu", "Kamis", "Jumat", "Sabtu"][new Date(`${targetDate}T00:00:00`).getDay()];
      const jadwalLibur = S.get("jadwalLibur") || {};
      const liburKu = jadwalLibur[userId];

      if (liburKu && liburKu === namaHariIni) {
        alert(`Akses ditolak!\n\nTanggal ${targetDate} jatuh pada hari ${namaHariIni} yang diset sebagai jadwal libur Anda. Anda tidak bisa melakukan Check-in dan Transaksi pada tanggal ini.`);
        return;
      }
      ...
      if (ex?.checkin_ts) {
        pushNotif("Absensi masuk untuk tanggal ini sudah ada.", "warning");
        return;
      }
    ...
    const doCheckout = () => {
      if (!userId) return;
      const all = S.get("absensi") || [];
      const d = txDate || today();
      const ex = all.find((a) => a.user_id === userId && a.date === d);
      if (!ex?.checkin_ts) {
        pushNotif("Belum ada check-in untuk tanggal ini.", "warning");
        return;
      }
      if (ex?.checkout_ts) {
        pushNotif("Check-out untuk tanggal ini sudah tercatat.", "warning");
        return;
      }
  ...
  function OwnerAbsensi({ pushNotif }) {
    ...
    return /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { className: "filter-bar mb8" }, /* @__PURE__ */ React.createElement("input", { type: "month", className: "inp inp-sm", value: month, onChange: (e) => setMonth(e.target.value) }), /* @__PURE__ */ React.createElement("select", { className: "inp inp-sm", value: selBranch, onChange: (e) => setSelBranch(e.target.value) }, /* @__PURE__ */ React.createElement("option", { value: "all" }, "Semua Cabang"), branches.map((b) => /* @__PURE__ */ React.createElement("option", { key: b.id, value: b.id }, b.name))), /* @__PURE__ */ React.createElement("button", { className: "btn-primary btn-sm", onClick: lockMonth }, "Kunci Rekap"), /* @__PURE__ */ React.createElement("button", { className: "btn-secondary btn-sm", onClick: unlockMonth }, "Buka Kunci")), /* @__PURE__ */ React.createElement("div", { className: "kpi-grid" }, /* @__PURE__ */ React.createElement("div", { className: "kpi-card kpi-omzet" }, /* @__PURE__ */ React.createElement("div", { className: "kpi-label" }, "Total Hadir"), /* @__PURE__ */ React.createElement("div", { className: "kpi-val" }, totalHadir, " hari")), /* @__PURE__ */ React.createElement("div", { className: "kpi-card kpi-profit" }, /* @__PURE__ */ React.createElement("div", { className: "kpi-label" }, "Total Jam"), /* @__PURE__ */ React.createElement("div", { className: "kpi-val" }, Math.round(totalMenit / 60 * 10) / 10, " jam")), /* @__PURE__ */ React.createElement("div", { className: "kpi-card kpi-cab" }, /* @__PURE__ */ React.createElement("div", { className: "kpi-label" }, "Pekerja"), /* @__PURE__ */ React.createElement("div", { className: "kpi-val" }, rows.length))), /* @__PURE__ */ React.createElement("h3", { className: "section-title mt12" }, "Detail Absensi Bulanan"), rows.length === 0 && /* @__PURE__ */ React.createElement("p", { className: "empty-txt" }, "Belum ada pekerja / data profiles belum termuat."), rows.map((r) => /* @__PURE__ */ React.createElement("div", { key: r.w.user_id, className: "peng-row", style: { flexDirection: "column", alignItems: "flex-start" } }, /* @__PURE__ */ React.createElement("div", { className: "peng-info", style: { width: "100%", marginBottom: "4px" } }, /* @__PURE__ */ React.createElement("span", { className: "peng-ket" }, r.w.display_name || r.w.email || r.w.user_id.slice(0, 6)), /* @__PURE__ */ React.createElement("span", { className: "peng-ts" }, "Cabang: ", branches.find((b) => b.id === r.w.branchId)?.name || r.w.branchId || "-", " | Hadir: ", r.hadir, " | Jam: ", Math.round(r.menit / 60 * 10) / 10, r.locked ? " | (Terkunci)" : "")), 
      /* @__PURE__ */ React.createElement("div", { style: { paddingLeft: "8px", borderLeft: "2px solid #555", width: "100%", marginTop: "4px" } }, 
        ...
        r.history.map(h => /* @__PURE__ */ React.createElement("div", { key: h.id, style: { fontSize: "12px", color: "#ccc", marginBottom: "2px" } }, 
  ...
  function OwnerSetting({ stab, setStab, pushNotif }) {
    ...
    return /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { className: "tabs tabs-sm" }, TABS.map((t) => /* @__PURE__ */ React.createElement("button", { key: t, className: "tab" + (stab === t ? " active" : ""), onClick: () => setStab(t) }, TLABEL[t]))), stab === "hpp" && /* @__PURE__ */ React.createElement(SettingHPP, { pushNotif }), stab === "paket" && /* @__PURE__ */ React.createElement(SettingPaket, { pushNotif }), stab === "cabang" && /* @__PURE__ */ React.createElement(SettingCabang, { pushNotif }), stab === "akun" && /* @__PURE__ */ React.createElement(SettingAkun, { pushNotif }), stab === "investor" && /* @__PURE__ */ React.createElement(SettingInvestor, { pushNotif }), stab === "branding" && /* @__PURE__ */ React.createElement(BrandingSetting, { pushNotif }), stab === "data" && /* @__PURE__ */ React.createElement(SettingData, { pushNotif }));
  }
  function BrandingSetting({ pushNotif }) {
    ...
    const saveManual = async () => {
      if (!logoUrl) {
        pushNotif("Isi URL logo dulu atau upload file.", "warning");
        return;
      }
      try {
        setBusy(true);
        await saveToDb(logoUrl);
        pushNotif("Logo berhasil disimpan.", "success");
      } catch (err) {
        pushNotif(err?.message || String(err), "warning");
      } finally {
        setBusy(false);
    ...
    return /* @__PURE__ */ React.createElement("div", null,
      /* @__PURE__ */ React.createElement("h3", { className: "section-title mt8" }, "Branding Logo"),
      /* @__PURE__ */ React.createElement("p", { className: "info-txt" }, "Upload logo baru ke bucket Supabase, atau isi URL manual jika file sudah ada di storage."),
      /* @__PURE__ */ React.createElement("div", { className: "form-card mt8" },
        /* @__PURE__ */ React.createElement("div", { style: { display: "flex", justifyContent: "center" } },
          /* @__PURE__ */ React.createElement("img", { src: logoUrl || getBrandLogo(), alt: "Logo bisnis", className: "brand-preview" })
        ),
        /* @__PURE__ */ React.createElement("div", { className: "field-group" },
          /* @__PURE__ */ React.createElement("label", null, "Upload Logo"),
          /* @__PURE__ */ React.createElement("input", { className: "inp", type: "file", accept: "image/*", onChange: doUpload, disabled: busy })
        ),
        /* @__PURE__ */ React.createElement("div", { className: "field-group" },
          /* @__PURE__ */ React.createElement("label", null, "URL Logo"),
          /* @__PURE__ */ React.createElement("input", { className: "inp", value: logoUrl, onChange: (e) => setLogoUrl(e.target.value), placeholder: "https://... atau public URL Supabase" })
        ),
        /* @__PURE__ */ React.createElement("button", { className: "btn-primary", onClick: saveManual, disabled: busy }, busy ? "Menyimpan..." : "Simpan Logo")
      )
    );
  }
  ...
  function EditMenuModal({ menu, bahan, isPaket, onSave, onClose }) {
    const [m, setM] = useState({ ...menu, tipe: isPaket ? "paket" : "satuan", resepBahanPokok: menu.resepBahanPokok || [], resepToping: menu.resepToping || [], imageUrl: menu.imageUrl || "", imagePath: menu.imagePath || "", boxCost: parseFloat(menu.boxCost || 0) || 0 });
    ...
    return /* @__PURE__ */ React.createElement(Modal, { title: (isPaket ? "Box - " : "Menu - ") + (m.id ? "Edit" : "Tambah"), onClose },
      /* @__PURE__ */ React.createElement("div", { className: "field-group" },
        /* @__PURE__ */ React.createElement("label", null, "Nama"),
        /* @__PURE__ */ React.createElement("input", { className: "inp", value: m.nama, onChange: (e) => setM((x) => ({ ...x, nama: e.target.value })) })
      ),
      /* @__PURE__ */ React.createElement("div", { className: "field-group" },
        /* @__PURE__ */ React.createElement("label", null, "Gambar Menu"),
        m.imageUrl && /* @__PURE__ */ React.createElement("img", { src: m.imageUrl, alt: m.nama || "Menu", className: "brand-preview", style: { width: 120, height: 120, margin: "0 auto" } }),
        /* @__PURE__ */ React.createElement("input", { className: "inp", type: "file", accept: "image/*", onChange: doUploadImage, disabled: uploading }),
        /* @__PURE__ */ React.createElement("input", { className: "inp", value: m.imageUrl || "", onChange: (e) => setM((x) => ({ ...x, imageUrl: e.target.value })), placeholder: "Atau tempel URL gambar menu" })
      ),
      isPaket && /* @__PURE__ */ React.createElement("div", { className: "field-group" },
        /* @__PURE__ */ React.createElement("label", null, "Isi Box (pcs)"),
        /* @__PURE__ */ React.createElement("input", { className: "inp", type: "number", value: m.isiBox || 3, onChange: (e) => setM((x) => ({ ...x, isiBox: parseInt(e.target.value) || 3 })) })
      ),
      isPaket && /* @__PURE__ */ React.createElement("div", { className: "field-group" },
        /* @__PURE__ */ React.createElement("label", null, "Harga Box"),
        /* @__PURE__ */ React.createElement("input", { className: "inp", type: "number", value: m.boxCost || 0, onChange: (e) => setM((x) => ({ ...x, boxCost: parseFloat(e.target.value) || 0 })) })
      ),
      /* @__PURE__ */ React.createElement("div", { className: "field-group" },
        /* @__PURE__ */ React.createElement("label", null, "Harga Jual"),
        /* @__PURE__ */ React.createElement("input", { className: "inp", type: "number", value: m.hargaJual, onChange: (e) => setM((x) => ({ ...x, hargaJual: parseFloat(e.target.value) || 0 })) })
      ),
      /* @__PURE__ */ React.createElement("h4", { className: "sub-title" }, "Resep Bahan Pokok ", isPaket ? "(total untuk " + (m.isiBox || 1) + " pcs)" : ""),
      ...
      /* @__PURE__ */ React.createElement("div", { className: "add-row" },
        /* @__PURE__ */ React.createElement("select", { className: "inp inp-sm", value: nRB.bahanId, onChange: (e) => setNRB((x) => ({ ...x, bahanId: e.target.value })) }, bahan.map((b) => /* @__PURE__ */ React.createElement("option", { key: b.id, value: b.id }, b.nama))),
        /* @__PURE__ */ React.createElement("input", { className: "inp inp-sm", type: "number", placeholder: "Gram", value: nRB.gram, onChange: (e) => setNRB((x) => ({ ...x, gram: e.target.value })) }),
        /* @__PURE__ */ React.createElement("button", { className: "btn-primary btn-sm", onClick: addRB }, "+")
      ),
      ...
      /* @__PURE__ */ React.createElement("div", { className: "add-row" },
        /* @__PURE__ */ React.createElement("input", { className: "inp inp-sm", placeholder: "Nama toping", value: nRT.nama, onChange: (e) => setNRT((x) => ({ ...x, nama: e.target.value })) }),
        /* @__PURE__ */ React.createElement("input", { className: "inp inp-sm", type: "number", placeholder: "Gram", value: nRT.gram, onChange: (e) => setNRT((x) => ({ ...x, gram: e.target.value })) }),
        /* @__PURE__ */ React.createElement("input", { className: "inp inp-sm", type: "number", placeholder: "Harga (Rp)", value: nRT.harga, onChange: (e) => setNRT((x) => ({ ...x, harga: e.target.value })) }),
        /* @__PURE__ */ React.createElement("button", { className: "btn-primary btn-sm", onClick: addRT }, "+")
      ),
      /* @__PURE__ */ React.createElement("div", { className: "hpp-preview" },
        "Bahan/pcs: ", /* @__PURE__ */ React.createElement("strong", null, fmtRp(info.bahanPokokPerPcs)),
        " | Varian/pcs: ", /* @__PURE__ */ React.createElement("strong", null, fmtRp(info.varianPerPcs)),
        " | HPP produk/pcs: ", /* @__PURE__ */ React.createElement("strong", null, fmtRp(info.produkPerPcs)),
        isPaket ? " | Total HPP box: " + fmtRp(info.totalBox) + " | Margin box: " + fmtRp(info.marginPerBox) : " | Margin/pcs: " + fmtRp(info.marginPerPcs)
      ),
      /* @__PURE__ */ React.createElement("div", { className: "row-wrap mt8" },
        /* @__PURE__ */ React.createElement("button", { className: "btn-secondary", onClick: onClose }, "Batal"),
        /* @__PURE__ */ React.createElement("button", { className: "btn-primary", onClick: () => {
          ...
          onSave(m);
        }, disabled: uploading }, uploading ? "Upload..." : "Simpan")
      )
    );
  }
